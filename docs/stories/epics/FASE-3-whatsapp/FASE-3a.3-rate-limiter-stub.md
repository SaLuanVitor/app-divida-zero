...
**Acceptance Criteria:**
- [x] `WhatsappRateLimiter` implementado com token bucket (burst=10, refill=1/s)
- [x] Rate limiter por telefone (refill=1/5s por destinatário)
- [x] `WhatsappChannel` usa rate limiter antes de "enviar"
- [x] Rate limiter retorna `{ allowed: bool, retry_after: seconds }`
- [ ] Testes de estresse do rate limiter (burst, refill, concorrência)
- [x] Configuração via ENV: `WHATSAPP_BURST_SIZE`, `WHATSAPP_REFILL_RATE`
