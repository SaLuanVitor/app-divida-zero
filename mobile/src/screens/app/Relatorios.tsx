...
const LegendPill = ({ color, label }: { color: string; label: string }) => (
  <View className="flex-row items-center rounded-full border border-slate-200 dark:border-slate-700 px-3 py-1 mr-2 mb-2">
    ...
    <AppText className="text-[11px] text-slate-600 dark:text-slate-200 font-semibold">{label}</AppText>
  </View>
);
      ...
      <Layout scrollable contentContainerClassName="p-4 bg-[#f8f7f5] dark:bg-black">
        ...
        <Card className="mb-3" noPadding><View className="p-4">
          ...
          <View className="flex-row items-center justify-between">
            <AppText className="text-xs text-slate-500 dark:text-slate-200">Competência por vencimento.</AppText>
            {isRefreshingPeriod ? (
        ...
        {isInitialEmptyLoading ? (
          <View className="py-2">
            ...
            <View className="items-center py-4">
              ...
              <AppText className="text-slate-500 dark:text-slate-200 text-xs mt-2">Atualizando dados...</AppText>
            </View>
          </View>
        ) : null}
          ...
          <>
            <Card className="mb-3" noPadding><View className="p-4">
              ...
              {data.monthly_trend.length === 0 ? (
                <AppText className="text-slate-500 dark:text-slate-200 text-sm">Sem dados para o gr\u00e1fico de tend\u00eancia.</AppText>
              ) : (
                <>
